...
export default function ListarReservas() {
  ...
  const [detallesId, setDetallesId] = useState<number | null>(null); // Estado para almacenar el ID de la reserva seleccionada
  const [detallesReserva, setDetallesReserva] = useState<any | null>(null); // Estado para almacenar los detalles de la reserva
  const [error, setError] = useState<string | null>(null); // Estado para manejar errores
  const [filtroEstado, setFiltroEstado] = useState<string>("todas"); // Estado para manejar el filtro de estado
  ...
  useEffect(() => {
    ...
    const fetchReservas = async () => {
      try {
        ...
        setReservas(data); // Suponiendo que la respuesta sea un array de reservas
        setLoading(false);
      } catch (error) {
  ...
  const verDetalles = async (id: number) => {
    // Si ya se muestran los detalles de esta reserva, los oculta
    if (detallesId === id) {
      setDetallesId(null);
    ...
    try {
      // Realizar la llamada para obtener los detalles de la reserva
  ...
  return (
    <div className="min-h-screen  p-8 max-w-screen-lg w-full mx-auto">
      ...
      <div className="mb-6">
        <label htmlFor="filtroEstado" className="mr-2 text-lg">Filtrar por estado:</label>
        <select
          id="filtroEstado"
          value={filtroEstado}
      ...
      {/* Card de detalles de reserva encima de la tabla */}
      {detallesId && detallesReserva && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-10">
          <div className="bg-white p-6 rounded-lg shadow-md max-w-lg w-full">
            ...
            <p><strong>ID:</strong> {detallesReserva.id}</p>
            <p><strong>Cliente:</strong> {detallesReserva.clientName}</p>
            <p><strong>Fecha:</strong> {new Date(detallesReserva.date).toLocaleDateString("es-PE")}</p>
            <p><strong>Hora:</strong> {new Date(detallesReserva.date).toLocaleTimeString("es-PE", { hour: '2-digit', minute: '2-digit' })}</p>

            <button
              onClick={() => setDetallesId(null)} // Oculta los detalles
            ...
            {reservasFiltradas.map((reserva) => (
              <tr key={reserva.id} className="border-b hover:bg-gray-50">
                ...
                <td className="py-2 px-4 text-center">
                  <button
                    onClick={() => verDetalles(reserva.id)} // Muestra los detalles al hacer clic
